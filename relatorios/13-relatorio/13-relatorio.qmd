...
# 1. Vetor com os dados (exemplo)
estado_civil <- c("Solteiro", "Casado", "Solteiro", "Divorciado", "Casado", 
                  "Solteiro", "Viúvo", "Casado", "Solteiro", "Divorciado")

# 2. Criar tabela de frequência e converter em data frame
freq_tabela <- as.data.frame(table(estado_civil))

# 3. Renomear colunas para melhor visualização
colnames(freq_tabela) <- c("Estado_Civil", "Frequencia")

# 4. Ordenar em ordem decrescente da frequência
freq_tabela <- freq_tabela[order(-freq_tabela$Frequencia), ]

# 5. Criar gráfico de barras horizontal com ggplot2
ggplot(freq_tabela, aes(x = reorder(Estado_Civil, Frequencia), y = Frequencia)) +
